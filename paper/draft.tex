...
%
This involves applying a learned model to make predictions for the
syntax of each item and then searching over the predictions to find a
consistent assignment for the entire sentence.
%
...
%
First, supertags are represented as vectors through embedding
techniques that enable to model to generalize naturally over complex, but
related structures.
%
Second, hierarchical dependence between choices is modeled using a recurrent tree
network that can capture long-distance influences as well as local ones.
%
...
To embed a supertag, we embed each node, group the resulting vectors
to form a tensor, and then summarize the tensor into a single vector
using a series of convolutional neural networks.
%
Since the weights of a convolutional neural network are optimized
during training, the data should determine which parts of the supertag
are important.
%
...
To explain our technique, we begin by observing that each elementary
tree is a complex structure with nodes labeled by category and
assigned a role that enables further tree operations.
%
...
\cline{3-4}
\multicolumn{2}{}{}& \multicolumn{2}{ |c| }{ \textbf{Accuracy}} \\ \hline
\textbf{Model} & \textbf{Embedding}  & \textbf{Before A*} & \textbf{After A*}  \\ \hline
Fergus-N &  Convolution &  58.17\% & 57.40\% \\ \cline{2-4}
         &  Token       &  60.69\% & 55.56\% \\ \cline{2-4}
         & Minimal Token & 52.09\% & 54.18\% \\ \cline{2-4}
\hline
Fergus-R &  Convolution &  67.62\% & 57.04\% \\ \cline{2-4}
         &  Token       &  82.65\% & 62.73\% \\ \cline{2-4}
         &  Minimal Token & 10.13\% & 19.66\% \\ \cline{2-4}
\hline
\end{tabular}
